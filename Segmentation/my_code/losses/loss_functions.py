...
class CEDiceLoss(DiceLoss):
    def __init__(self, mode, smooth_factor=0.1):
        super().__init__(mode)
        self.smooth_factor = smooth_factor
        # self.ce = nn.CrossEntropyLoss()
        self.labsm = smp.losses.SoftCrossEntropyLoss(smooth_factor=self.smooth_factor)

    def forward(self, y_pred, y_true):
        dice = super().forward(y_pred, y_true)
        # ce = self.ce(y_pred, y_true)
        labsm = self.labsm(y_pred, y_true)
        return (0.75*ce) + (0.25*dice)
