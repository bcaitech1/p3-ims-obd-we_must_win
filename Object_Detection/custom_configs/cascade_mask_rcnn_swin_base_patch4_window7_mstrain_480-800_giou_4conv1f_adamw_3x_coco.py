model = dict(
    ...
    train_cfg=dict(
        ...
        rpn_proposal=dict(nms_across_levels=False, nms_pre=2000, nms_post=2000, max_per_img=2000, nms=dict(type="nms", iou_threshold=0.7), min_bbox_size=0),
        rcnn=[
            dict(
    ...
    test_cfg=dict(
        rpn=dict(nms_across_levels=False, nms_pre=1000, nms_post=1000, max_per_img=1000, nms=dict(type="nms", iou_threshold=0.7), min_bbox_size=0),
        ...
        pipeline=[
            ...
            dict(
                ...
                policies=[
                    ...
                    [
                        {"type": "Resize", "img_scale": [(400, 1333), (500, 1333), (600, 1333)], "multiscale_mode": "value", "keep_ratio": True},
                        {"type": "RandomCrop", "crop_type": "absolute_range", "crop_size": (384, 600), "allow_negative_crop": True},
                        {
                            "type": "Resize",
                            "img_scale": [
                                (480, 1333),
...
optimizer = dict(
    ...
    weight_decay=0.05,
    paramwise_cfg=dict(
        custom_keys=dict(absolute_pos_embed=dict(decay_mult=0.0), relative_position_bias_table=dict(decay_mult=0.0), norm=dict(decay_mult=0.0))
    ),
)
optimizer_config = dict(grad_clip=None)
lr_config = dict(policy="step", warmup="linear", warmup_iters=500, warmup_ratio=0.001, step=[27, 33])
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(
            ...
            init_kwargs=dict(project="pstage-3-od", name="cascade_mask_rcnn_swin_base_patch4_window7_mstrain_480-800_giou_4conv1f_adamw_3x_coco"),
        ),
    ],
)
...
log_level = "INFO"
load_from = "/opt/ml/pstage_3/mmdetection/pretrained/cascade_mask_rcnn_swin_base_patch4_window7_mstrain_480-800_giou_4conv1f_adamw_3x_coco.pth"
resume_from = None
workflow = [("train", 1)]
fp16 = None
seed = 42
gpu_ids = [0]
work_dir = "./save_dir/cascade_mask_rcnn_swin_base_patch4_window7_mstrain_480-800_giou_4conv1f_adamw_3x_coco"
